...
JSON API Endpoints:
  - GET  /api/status  : Returns server status, log counts, and sliding window state.
  - GET  /api/logs    : Returns list of real-time server log entries.
  - POST /api/send    : Sends a legitimate UDP packet ('position' or 'score').
  - POST /api/attack  : Fires an attack ('tamper', 'replay', 'spoof') over UDP.
  - POST /api/burst   : Sends a 20-packet traffic burst (9:1 Low:High ratio).
...
class ARSpeckHTTPHandler(http.server.SimpleHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if path == "/api/send":
            ...
            speck_client.seq_no += 10 # Advance client seq to avoid collisions
            ...
            if speck_client is None:
                speck_client = ARSpeckClient(server_host="127.0.0.1", server_port=UDP_PORT, key=KEY)
            sent = speck_client.send_burst(count=count, interval_ms=10.0)
            self._send_json_response({
                "status": "success",
